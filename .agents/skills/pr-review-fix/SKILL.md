...
3. Fetch PR metadata plus review comments when a PR exists. Prefer a thread-aware API such as
   `gh api graphql` when resolution state matters; use an available GitHub connector or report
   the missing API prerequisite when no suitable client is available.
...
8. Commit review fixes separately from workflow/process documentation when the repository uses
   version control; otherwise report the validated but uncommitted state.
...
- First reviewer pass: fix all confirmed `must-fix` and in-scope `should-fix` comments.
- Second reviewer pass: fix new correctness, spec, security/privacy, data-loss, accessibility, or low-risk maintainability issues.
...
- Do not chase zero reviewer comments when CI, local validation, and required QA are green and remaining comments are stale, duplicate, cosmetic, low-confidence, or already tracked elsewhere.
...
- Do not treat automated review comments as merge-blocking approvals or requested changes.
- Do not mix follow-up feature work into the review-fix commit.
- Do not request repeated reviewer re-reviews after the review loop budget is exhausted unless a `must-fix` or explicitly accepted `should-fix` remains.
