...
let tmpDir: string;
const originalBundleRoot = process.env.AGENT_STEWARD_PROJECT_BUNDLE_ROOT;
const originalBackupRoot = process.env.AGENT_STEWARD_BACKUP_ROOT;

beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "asm-project-bundle-"));
  process.env.AGENT_STEWARD_PROJECT_BUNDLE_ROOT = path.join(tmpDir, "project-bundles");
  process.env.AGENT_STEWARD_BACKUP_ROOT = path.join(tmpDir, "backups");
  await fs.mkdir(process.env.AGENT_STEWARD_BACKUP_ROOT, { recursive: true });
});

afterEach(async () => {
  vi.restoreAllMocks();
  if (originalBundleRoot === undefined) {
    delete process.env.AGENT_STEWARD_PROJECT_BUNDLE_ROOT;
  } else {
    process.env.AGENT_STEWARD_PROJECT_BUNDLE_ROOT = originalBundleRoot;
  }
  if (originalBackupRoot === undefined) {
    delete process.env.AGENT_STEWARD_BACKUP_ROOT;
  } else {
    process.env.AGENT_STEWARD_BACKUP_ROOT = originalBackupRoot;
  }
...
async function writeBackupPackage(input: {
  ...
  const backupDir = path.join(process.env.AGENT_STEWARD_BACKUP_ROOT!, input.backupId);
...
describe("project bundle service", () => {
  ...
  it("blocks validation when the bundle root path is not a writable directory", async () => {
    const bundleRoot = process.env.AGENT_STEWARD_PROJECT_BUNDLE_ROOT!;
  ...
  it("blocks validation when a missing output root has a non-directory ancestor", async () => {
    ...
    process.env.AGENT_STEWARD_PROJECT_BUNDLE_ROOT = path.join(blockedAncestor, "project-bundles");
  ...
  it("blocks validation when a missing output root has a broken symlink ancestor", async () => {
    ...
    process.env.AGENT_STEWARD_PROJECT_BUNDLE_ROOT = path.join(brokenAncestor, "project-bundles");
  ...
  it("blocks validation when an existing output root ancestor is not accessible", async () => {
    ...
    process.env.AGENT_STEWARD_PROJECT_BUNDLE_ROOT = path.join(inaccessibleAncestor, "project-bundles");
