...
let tmpDir: string;
const origEnv = process.env.AGENT_STEWARD_CONFIG_PATH;

beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "asm-config-test-"));
  process.env.AGENT_STEWARD_CONFIG_PATH = path.join(tmpDir, "config.json");
});

afterEach(async () => {
  if (origEnv === undefined) {
    delete process.env.AGENT_STEWARD_CONFIG_PATH;
  } else {
    process.env.AGENT_STEWARD_CONFIG_PATH = origEnv;
  }
