...
本文档用于记录 **Antigravity**, **Windsurf**, **Codex** 与 **Cursor** 在 macOS 上的本地存储布局、关键文件、以及我们在实现/调试 `agent-steward` 过程中验证过的“可用事实”。它是一个活文档：每次发现新结构或出现不兼容变更时，都应补充到这里（最好附上“如何复现/如何验证”）。
...
`agent-steward` 当前对 Cursor 的支持边界是：
...
因此 `agent-steward` 对 Antigravity 采用 Windsurf 类似的 attach：
...
- 让 `agent-steward` 走 “Attach 模式（Antigravity.log）” 重新定位 pid/ports。
...
| Codex state DB | `~/.codex/state_5.sqlite` — auto-generated titles, cwd, first_user_message, archived flag（title 列从不因用户重命名而更新） |
| Codex session index | `~/.codex/session_index.jsonl` — 权威标题来源，含用户手动重命名（append-only，最后一行赢） |
...
2. **直接编辑 config.json**：修改 `~/.agent-steward/config.json`（或 `$AGENT_STEWARD_CONFIG_PATH`），在 `roots` 数组中追加条目：
...
- 2026-04-08（第一次）：确认 `~/.codex/state_5.sqlite` `threads.title` 为自动生成标题的持久化来源（Rust app-server 通过 sqlx 写入）；`rollout_path` 为 JSONL 绝对路径。实现改为优先从 SQLite 读取，JSONL 流式解析（256 KB 逐行预算）作为 fallback。
- 2026-04-08（第二次）：通过逆向 Codex Rust 二进制确认 `threads.title` 从不被重命名操作更新（`strings` 确认不存在 `UPDATE threads SET title`）；发现 `~/.codex/session_index.jsonl` 为手动重命名的权威来源（`set_thread_name` handler append-only 写入，last-wins per UUID）。标题读取升级为三层策略：`session_index.jsonl` → `state_5.sqlite` → JSONL 流式解析。详见 [`docs/storage/codex-storage-notes.md`](codex-storage-notes.md)。
