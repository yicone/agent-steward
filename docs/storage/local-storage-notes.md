...
本文档用于记录 **Antigravity**, **Windsurf**, **Codex** 与 **Cursor** 在 macOS 上的本地存储布局、关键文件、以及我们在实现/调试 `agent-switch` 过程中验证过的“可用事实”。它是一个活文档：每次发现新结构或出现不兼容变更时，都应补充到这里（最好附上“如何复现/如何验证”）。
...
`agent-switch` 当前对 Cursor 的支持边界是：
...
因此 `agent-switch` 对 Antigravity 采用 Windsurf 类似的 attach：
...
- 让 `agent-switch` 走 “Attach 模式（Antigravity.log）” 重新定位 pid/ports。
...
2. **直接编辑 config.json**：修改 `~/.agent-switch/config.json`（或 `$AGENT_SWITCH_CONFIG_PATH`），在 `roots` 数组中追加条目：
