...
本文档记录 **Codex CLI / Codex App (macOS)** 在本地磁盘上的会话存储布局、标题存储机制，以及 `agent-steward` 在集成过程中验证过的"可用事实"。
...
`agent-steward` 的处理：`isInjectedContextMessage()` + `extractCodexTitle()` in `src/lib/parse/codexLog.ts`。
