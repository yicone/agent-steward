...
> 本文档对应 issue [#21](https://github.com/yicone/agent-steward/issues/21) 的设计要求，同时也是 issue [#5](https://github.com/yicone/agent-steward/issues/5)（Architecture Review v1: Search + structured filters）的实现说明文档。
...
**索引策略**：
- 索引文件：`~/.agent-steward/search.db`（可通过 `AGENT_STEWARD_SEARCH_DB_PATH` 覆盖）
