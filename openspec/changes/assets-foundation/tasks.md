...
- [ ] 1.1 Define reusable context asset TypeScript types for subtype, scope, source, status, provenance, and optional in-effect metadata.
- [ ] 1.2 Add a bounded local asset data provider or seed adapter that does not imply complete cross-agent normalization.
...
- [ ] 2.4 Implement selected asset detail with identity, subtype, scope, source, status, provenance summary, and bounded evidence routes.
- [ ] 2.5 Implement in-effect/usage rendering for available applicability data and explicit unavailable messaging when applicability is unknown.

## 3. Routed Handoff

- [ ] 3.1 Add shell-level routed handoff support into Assets for subtype, scope, object selection, issue, and return context.
